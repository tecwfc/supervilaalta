// Função segura para formatar data (evita erros)
...
// ─── ARMAZENAMENTO LOCAL PARA HORAS ──────────────────────────────────────
let horasLocais = JSON.parse(localStorage.getItem('supervila_horas') || '{}');

// Funções auxiliares para horas locais
function salvarHoraLocal(id, hora) {
  if (!id || !hora) return;
  horasLocais[id] = hora;
  localStorage.setItem('supervila_horas', JSON.stringify(horasLocais));
}

function obterHoraLocal(id) {
  return horasLocais[id] || '';
}

function limparHorasOrfas() {
  if (!dadosCache.lista) return;

  const idsValidos = new Set(dadosCache.lista.map(item => item[0]));

  Object.keys(horasLocais).forEach(id => {
    if (!idsValidos.has(id)) {
      delete horasLocais[id];
    }
  });
...
function parseDate(raw) {
  ...
  try {
    ...
    if (typeof raw === 'number' || (!isNaN(raw) && typeof raw === 'string' && !raw.includes('/') && !raw.includes('-'))) {
...
function getCurrentDateTime() {
  ...
  // Formatar hora no fuso de Brasília
  const horaSP = now.toLocaleTimeString('pt-BR', {
    timeZone: 'America/Sao_Paulo',
    hour: '2-digit',
    minute: '2-digit',
    hour12: false
  ...
  console.log(`Hora local do sistema: ${now.getHours()}:${now.getMinutes()}`);
  console.log(`Hora convertida para SP: ${horaSP}`);

  return {
    data: `${dia}/${mes}/${ano}`,
    hora: horaSP,
    dataHora: `${dia}/${mes}/${ano} ${horaSP}`,
...
function renderCards(lista, saldoPrevio) {
  ...
  listaOrdenada.forEach((item) => {
    ...
    // No renderCards, a parte da hora deve ser:
    let horaExib = '';

    // 1. Prioridade: hora da planilha (índice 5)
    if (item[5] && item[5] !== '') {
      // Se veio como string HH:MM, usar direto
      if (typeof item[5] === 'string' && item[5].includes(':')) {
        horaExib = item[5];
      } else {
        // Tentar converter se veio em outro formato
        horaExib = String(item[5]).trim();
      }
      console.log(`Hora da planilha para ${id}: ${horaExib}`);
    }
    // 2. Fallback: localStorage
    else {
      horaExib = obterHoraLocal(id);
      if (horaExib) {
        console.log(`Hora do localStorage para ${id}: ${horaExib}`);
      }
    }

    // Se tem hora, mostrar
    const horaHTML = horaExib ? `<span class="hora-icon">🕐</span> ${horaExib}` : '';
...
async function lancar() {
  ...
  console.log(`NOVO LANÇAMENTO - Hora real: ${agora.hora}`);
  ...
  try {
    ...
    if (data && data.status === "sucesso") {
      ...
      // IMPORTANTE: Aguardar a tabela ser atualizada para pegar o ID real
      setTimeout(async () => {
        // Atualizar a tabela primeiro
        await atualizarTabela();

        // Agora procurar o registro que acabamos de lançar
        if (dadosCache.lista && dadosCache.lista.length > 0) {
          // Pegar o primeiro registro (mais recente)
          const ultimoRegistro = dadosCache.lista[0];
          const idReal = ultimoRegistro[0];

          console.log(`ID real do registro: ${idReal}`);
          console.log(`Salvando hora no localStorage com ID real: ${idReal} = ${agora.hora}`);

          // Salvar hora com o ID real
          salvarHoraLocal(idReal, agora.hora);

          // Atualizar a tela novamente para mostrar a hora
          renderCards(dadosCache.lista, dadosCache.saldoPrevio);
        }
      }, 1000);
...
async function abrirModal(id, data, desc, rec, pag) {
  ...
  // === REMOVIDO: Campo de hora não é mais criado ===
  // Verificar se existe campo de hora e removê-lo
  const horaFieldExistente = document.getElementById("editHora");
  if (horaFieldExistente) {
    const horaDiv = horaFieldExistente.parentNode;
    if (horaDiv) {
      horaDiv.remove();
    }
  }
...
async function salvarEditar() {
  ...
  // === REMOVIDO: Não usa mais hora na edição ===
  // Manter a hora original do registro
  ...
  try {
    const response = await fetch(API_URL, {
      ...
      body: JSON.stringify({
        ...
        recebido: tipo === "recebido" ? valorNum : 0,
        pago: tipo === "pago" ? valorNum : 0,
        // NOTA: Não enviamos hora para não alterar
      }),
    });
    ...
    if (data && data.status === "ok") {
      // NOTA: Não atualizamos a hora local na edição
...
async function gerarPDFLivroCaixa() {
  ...
  try {
    ...
    // Dentro de gerarPDFLivroCaixa(), na parte onde formata as datas:
...
//setTimeout(resetarHorasAntigas, 3000);
